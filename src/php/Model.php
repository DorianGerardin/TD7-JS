...
class Model {
    ...
    public static function selectAdherentByLivre($livre) {
        
        try {
            // préparation de la requête
            $sql = "SELECT A.*, E.idLivre
                    FROM emprunt E RIGHT OUTER JOIN adherent A ON A.idAdherent = E.idAdherent
                                               LEFT OUTER JOIN livre L ON L.idLivre = E.idLivre
                    WHERE E.idLivre = :livre;";
            $req_prep = self::$pdo->prepare($sql);
            // passage de la valeur de name_tag
            $values = array("livre" => $livre);
            // exécution de la requête préparée
            $req_prep->execute($values);
            $results = $req_prep->fetchAll();
            return $results;
    ...
    public static function rendreLivre($adherent, $livre) {
        
        try {
            // préparation de la requête
            $sql = "DELETE FROM emprunt WHERE idAdherent = :adherent AND idLivre = :livre";
            $req_prep = self::$pdo->prepare($sql);
            // passage de la valeur de name_tag
            $values = array("adherent" => $adherent,
                            "livre" => $livre);
            // exécution de la requête préparée
            $req_prep->execute($values);
            $tabResults = self::selectLivresEmpruntes();
            return $tabResults;
        } catch (PDOException $e) {
            echo $e->getMessage();
            die("Erreur lors de la recherche dans la base de données.");
    ...
    public static function emprunterLivre($adherent, $livre) {
